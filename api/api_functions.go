...
type ApiCreateTransformationFunctionInstanceRequest struct {
	ctx                                            context.Context
	ApiService                                     *FunctionsAPIService
	createTransformationFunctionInstanceAlphaInput *CreateTransformationFunctionInstanceAlphaInput
}

func (r ApiCreateTransformationFunctionInstanceRequest) CreateTransformationFunctionInstanceAlphaInput(
	createTransformationFunctionInstanceAlphaInput CreateTransformationFunctionInstanceAlphaInput,
) ApiCreateTransformationFunctionInstanceRequest {
	r.createTransformationFunctionInstanceAlphaInput = &createTransformationFunctionInstanceAlphaInput
	return r
}

func (r ApiCreateTransformationFunctionInstanceRequest) Execute() (*CreateTransformationFunctionInstance200Response, *http.Response, error) {
	return r.ApiService.CreateTransformationFunctionInstanceExecute(r)
}

/*
CreateTransformationFunctionInstance Create Transformation Function Instance

Creates a Transformation Function instance connected to the given Integration.

This endpoint is specifically designed for Transformations and includes integration_type as a mandatory field.

• In order to successfully call this endpoint, the specified Workspace needs to have the Functions feature enabled. Please reach out to your customer success manager for more information.

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ApiCreateTransformationFunctionInstanceRequest
*/
func (a *FunctionsAPIService) CreateTransformationFunctionInstance(
	ctx context.Context,
) ApiCreateTransformationFunctionInstanceRequest {
	return ApiCreateTransformationFunctionInstanceRequest{
		ApiService: a,
...
func (a *FunctionsAPIService) CreateTransformationFunctionInstanceExecute(
	r ApiCreateTransformationFunctionInstanceRequest,
) (*CreateTransformationFunctionInstance200Response, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodPost
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *CreateTransformationFunctionInstance200Response
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(
		r.ctx,
		"FunctionsAPIService.CreateTransformationFunctionInstance",
	)
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/transformation-function-instances"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.createTransformationFunctionInstanceAlphaInput == nil {
		return localVarReturnValue, nil, reportError(
			"createTransformationFunctionInstanceAlphaInput is required and must be specified",
		)
	}
	...
	// body params
	localVarPostBody = r.createTransformationFunctionInstanceAlphaInput
	req, err := a.client.prepareRequest(
		r.ctx,
		localVarPath,
		localVarHTTPMethod,
