...
type ApiHandleWebhookRequest struct {
	ctx                context.Context
	ApiService         *FunctionsApiService
	handleWebhookInput *HandleWebhookInput
}

func (r ApiHandleWebhookRequest) HandleWebhookInput(
	handleWebhookInput HandleWebhookInput,
) ApiHandleWebhookRequest {
	r.handleWebhookInput = &handleWebhookInput
	return r
}

func (r ApiHandleWebhookRequest) Execute() (*HandleWebhook200Response, *http.Response, error) {
...
Handles Function webhook calls.

• In order to successfully call this endpoint, the specified Workspace needs to have the Functions feature enabled. Please reach out to your customer success manager for more information.

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ApiHandleWebhookRequest
*/
func (a *FunctionsApiService) HandleWebhook(ctx context.Context) ApiHandleWebhookRequest {
	return ApiHandleWebhookRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
func (a *FunctionsApiService) HandleWebhookExecute(
	r ApiHandleWebhookRequest,
) (*HandleWebhook200Response, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodPost
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *HandleWebhook200Response
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(
		r.ctx,
		"FunctionsApiService.HandleWebhook",
	)
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/functions/webhook"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.handleWebhookInput == nil {
		return localVarReturnValue, nil, reportError(
			"handleWebhookInput is required and must be specified",
		)
	}
	...
	// body params
	localVarPostBody = r.handleWebhookInput
	req, err := a.client.prepareRequest(
		r.ctx,
		localVarPath,
		localVarHTTPMethod,
