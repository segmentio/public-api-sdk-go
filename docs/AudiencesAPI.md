...
> GetAudienceEcho200Response GetAudienceEcho(ctx, spaceId).Message(message).Execute()
...
func main() {
    spaceId := "9aQ1Lj62S4bomZKLF4DPqW" // string | 
    message := "Hello from test!" // string | Optional message to echo back.  This parameter exists in alpha. (optional)

    configuration := api.NewConfiguration()
    apiClient := api.NewAPIClient(configuration)
    token := "<BEARER_TOKEN>"
    ctx := context.WithValue(context.Background(), api.ContextAccessToken, token)
    resp, r, err := apiClient.AudiencesAPI.GetAudienceEcho(ctx, spaceId).Message(message).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `AudiencesAPI.GetAudienceEcho``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
        responseErrors := api.UnwrapFullErrors(err)
        if responseErrors != nil {
            for _, responseError := range responseErrors.Errors {
    ...
    // response from `GetAudienceEcho`: GetAudienceEcho200Response
    fmt.Fprintf(os.Stdout, "Response from `AudiencesAPI.GetAudienceEcho`: %v\n", resp.GetData())
}
```
...
Other parameters are passed through a pointer to a apiGetAudienceEchoRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------

 **message** | **string** | Optional message to echo back.  This parameter exists in alpha. | 
